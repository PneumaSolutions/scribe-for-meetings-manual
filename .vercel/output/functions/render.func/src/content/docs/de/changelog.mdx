...
Die RIM-Versionen werden durch Überschriften auf Ebene 2 gekennzeichnet. Umfangreiche Aktualisierungen mit mehreren Funktionen für beide Plattformen werden in Mac- und Windows-Unterüberschriften auf Ebene 3 unterteilt.
## 3.2.13
## Windows
Es wurde ein Problem mit der Tastatureingabe behoben, wenn eine Verbindung zu Zielrechnern mit JAWS 2024 hergestellt wurde.
## 3.2.12
Ein seltenes Problem wurde behoben, das verhindern konnte, dass sich die RIM-App mit dem Cloud-Dienst verbindet.
## 3.2.11
...
## 3.2.9
### macOS
Wir sind leider nicht mehr in der Lage, die automatische Stereomikrofoneingabe in Sprachkonversationen auf dieser Plattform anzubieten, da dies in einigen Fällen die Mikrofoneingabe auf einem Mac überhaupt nicht funktionierte. Bitte beachten Sie, dass diese Änderung keinen Einfluss auf die Audioausgabe auf dem Zielrechner hat; diese ist immer noch Stereo.
## 3.2.8
## macOS
Es wurde ein Fehler behoben, der verhinderte, dass RIM angezeigt wurde, wenn die Anwendung über den Finder oder das Dock aktiviert wurde, anstatt den globalen Hotkey zu verwenden.
## 3.2.7
Sprachkonversationen verwenden nun automatisch Stereo, wenn die Audio-Hardware dies unterstützt. Dies ist nützlich, wenn RIM für die Ausbildung in der Audioproduktion verwendet wird.
## 3.2.6
Ein kürzlich aufgetretenes Problem wurde behoben, das verhindern konnte, dass RIM bei einer Neuinstallation des Betriebssystems funktioniert.
## 3.2.5
...
* RIM für Mac passt nun die Menüleiste an, und insbesondere die Option "Über" im Anwendungsmenü öffnet nun unseren eigenen Über-Dialog anstelle des generischen Dialogs.

## 3.1.47
### Windows
Wenn ein Mac-Controller an ein Windows-Ziel angeschlossen wird und beide Seiten das gleiche, nicht-US-amerikanische Tastaturlayout verwenden, wurden die Tastatureingaben des Controllers nicht korrekt übersetzt. Version 3.1.47 für Windows behebt dieses Problem.
## 3.1.46
...
## 3.1.40
Fügt weitere Protokollierung in Bezug auf Audio auf dem Ziel hinzu. Wenn Sie also verzögertes Audio auf dem Zielgerät reproduzieren können, tun Sie dies bitte mit dieser Version und senden Sie dann die Protokolle. 
## 3.1.39
### Mac
...
die Gesamtzahl der Berechtigungsschritte nicht erhöht. Aber für Updates,
...
version 3.1.37 laufen müssen, um die volle Funktionalität bei der Verbindung mit einem Mac-Ziel zu erhalten.
## 3.1.36
## Windows
...
* Bei der Verwendung von RIM für Mac als Controller mit der Fernzugriffsfunktion verwendet RIM nun die Stimme und die Sprechgeschwindigkeit, die im Abschnitt Gesprochener Inhalt der Systemeinstellungen definiert sind.

## 3.1.34
### Mac
Wenn ein Mac-Controller eine Verbindung zu einem Windows-Ziel herstellt, auf dem kein Screenreader läuft, können Sie nun das Modul für die Fernzugänglichkeit auf dem Windows-Ziel ausführen und die Sprachausgabe auf dem Mac-Controller erhalten. Beachte, dass du immer noch NVDA-Tastaturbefehle verwenden musst, wenn du mit dem Windows-Ziel interagierst, nicht VoiceOver-Befehle, und dass wir das NVDA-Laptop-Tastaturlayout verwenden. 
## 3.1.33
...
## 3.1.24
* Wenn eine Verbindung zu einem Zielrechner mit einer geringeren Bildschirmauflösung oder einem höheren DPI-Faktor (Dots per Inch) als der Controller hergestellt wird, skaliert RIM nun die Anzeige des entfernten Zielrechners so, dass sie auf den Bildschirm des Controllers passt, was die Arbeit mit dem entfernten Rechner visuell erleichtert.
...
## 3.1.21
* Es wurde eine neue Tastenkombination, Windows+Umschalt+M, hinzugefügt, um in einem Sitzungsfenster das Fenster zu minimieren und den Tastaturfokus auf den lokalen Rechner (Controller) zurückzubringen.
...
## 3.1.19
* Es wurden einige Fehler bei der Behandlung von entfernten Tastatureingaben behoben. Insbesondere wenn JAWS auf dem Zielcomputer läuft, erkennt JAWS nun korrekt das Drücken der Pause- und NumLock-Taste auf dem Controller. Bitte lassen Sie uns wissen, wenn Sie Regressionen bei der Behandlung von Tastatureingaben finden, insbesondere bei nicht-amerikanischen Tastaturlayouts.
...
## 3.1.16
* Wenn JAWS auf dem Zielrechner mit aktivierter Unified Keyboard Processing Funktion läuft, reagiert JAWS nun zuverlässig auf Tastendrücke vom Controller.
* Es wurde ein Fehler behoben, der sporadisch auftrat, wenn das Add-on für die Fernzugänglichkeit gestartet wurde, nachdem das NVDA Add-on auf dem Controller-Rechner installiert oder aktualisiert wurde.

## 3.1.15
* Wenn Sie eine Verbindung zu einem Zielrechner herstellen, auf dem kein Bildschirmleseprogramm läuft, aktiviert RIM automatisch die Fernzugänglichkeit, wenn Sie ein Bildschirmleseprogramm auf dem Controller-Rechner verwenden. Wenn Sie einen anderen Bildschirmleser als NVDA auf dem Controller-Rechner verwenden, wird RIM diesen Bildschirmleser sogar automatisch beenden und NVDA starten, wenn Sie ihn installiert haben.
...
## 3.1.11
* Wenn auf dem Zielrechner mindestens diese Version von RIM läuft, aber noch nicht auf die neueste Version aktualisiert wurde, kann der Controller nun den Zielrechner dazu zwingen, seine RIM-Installation über eine neue Option im Sitzungsmenü zu aktualisieren.
...
* Das Fenster Verbindungsdetails wurde verbessert. Die Statistiken werden live aktualisiert, solange das Fenster geöffnet ist, und das Fenster enthält nun auch Informationen über den Zielcomputer.
...
## 3.1.8
* Es wurde ein Problem behoben, das manchmal den Start des Fernzugriffsmoduls verhinderte.
...
## 3.0.8
Es wurde ein Fehler behoben, der manchmal verhinderte, dass die Bildschirmausgabe aus der Ferne funktionierte, wenn eine Aufforderung zur Benutzerkontensteuerung angezeigt wurde.
## 3.0.7
...
## 3.0.3
* RIM zeigt nicht mehr fälschlicherweise die Schaltfläche "Create an Installer for Unattended Access" an, wenn Sie kein Abonnement haben.
...
## 3.0.0
Mit dieser Version ist die öffentliche Betaversion von RIM abgeschlossen. Die RIM-Preispläne und die Einschränkungen für die kostenlose Nutzung sind nun in Kraft. Für kostenlose Konten zeigt RIM jetzt einen Nutzungszähler zusammen mit einer Kaufschaltfläche an.
## 0.12.2
...
## 0.11.6
* RIM hat jetzt eine neue Option, um einen Rechner für den unbeaufsichtigten Zugriff unter Ihrem RIM-Konto zu registrieren, ohne eine Remote-Sitzung mit einem Schlüsselwort starten zu müssen. Drücken Sie auf dem Rechner, den Sie registrieren möchten, die neue Schaltfläche "Diesen Rechner für den unbeaufsichtigten Zugriff registrieren" auf dem Bildschirm "Hilfe erhalten". Melden Sie sich dann bei Ihrem RIM-Konto an, falls Sie dies noch nicht getan haben, geben Sie einen Namen für den Rechner ein und fertig. Beachten Sie, dass wir noch an einer Möglichkeit arbeiten, das RIM-Installationsprogramm anzupassen, um eine Massenverteilung zu ermöglichen und die Einrichtung des unbeaufsichtigten Zugriffs auf Rechnern, auf die Sie noch keinen Zugriff haben, so einfach wie möglich zu gestalten.
...
## 0.11.4
Dieses Update behebt einige Fehler beim Neustart und der Wiederverbindung. Wir haben noch nicht alle gemeldeten Fehler diagnostiziert, aber wir konnten bei unseren eigenen Tests einige Probleme finden und beheben.
## 0.11.3
...
## 0.11.1
Dies ist ein Update zur Fehlerbehebung. Bitte beachten Sie, dass unbeaufsichtigte Ziele weiterhin manuell aktualisiert werden müssen, indem Sie das neueste RIM-Installationsprogramm ausführen. Allerdings können Sie nun nach der Installation eine Verbindung zu einem unbeaufsichtigten Ziel herstellen, ohne dass jemand das Fenster "Hilfe erhalten" schließen muss.
## 0.11.0
* Die Funktion "Reboot and Reconnect" ist nun implementiert. Mit der neuen Option "Reboot and Reconnect" (Neustart und Wiederverbindung) im RIM-Menü des Controllers können Sie entweder einen Standard- oder einen Notfall-Neustart durchführen und die Sitzung automatisch fortsetzen, wenn der Neustart abgeschlossen ist. RIM stellt die Verbindung auch dann wieder her, wenn der Zielcomputer aus einem anderen Grund neu gestartet wird, z. B. wenn ein Installationsprogramm den Computer neu startet.
...
## 0.10.9
RIM funktioniert jetzt auch auf einem Zielrechner, der kein Audiogerät hat. In Kombination mit dem bestehenden Modul für Fernzugriff eröffnet dies den Zugang zu Servern und virtuellen Maschinen, auf die bisher nur über RDP und dessen sehr suboptimale Audioausgabe mit einem Bildschirmlesegerät zugegriffen werden konnte.
## 0.10.8
...
## 0.10.5
Diese Version enthält einige Änderungen unter der Haube. Insbesondere haben wir die Installation und die zugrunde liegenden Pakete aktualisiert, um Probleme zu beheben, die dazu führten, dass die Installation hängen blieb.
## 0.10.4
