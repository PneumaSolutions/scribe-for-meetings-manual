...
Si votre entreprise affecte un technicien d'assistance à un ensemble spécifique de machines, vous souhaitez probablement vous assurer qu'il n'a accès qu'à cet ensemble spécifique. C'est là qu'intervient le paramètre de contrôle d'accès pour les groupes cibles.  
...
1. Il vous sera d'abord demandé si vous souhaitez que cette machine soit configurée pour un accès entièrement sans surveillance ou pour un accès guidé dans lequel l'utilisateur doit accepter une invite pour initier la connexion.
...
1. Une case à cocher vous permet ensuite d'opter pour une notification par courrier électronique lorsqu'un utilisateur installe RIM via ce programme d'installation.
...
Une installation silencieuse peut être lancée en exécutant le programme d'installation exécutable avec le paramètre de ligne de commande */S*. Cette fonction est utile pour installer RIM dans le cadre d'un déploiement de masse.
...
Vous pouvez facilement modifier votre abonnement. Si vous bénéficiez d'un abonnement mensuel, le montant restant sera calculé au prorata du nouveau montant, et le nouveau montant total sera facturé pour les mois suivants. En cliquant sur "Upgrade Subscription", vous accédez à une page presque identique à la page de sélection du plan initial. Il existe quelques différences notables que nous décrivons ci-dessous :
...
* Les plans d'entreprise, y compris l'addon d'entreprise, ne peuvent pas être acquis directement sur cette page.
