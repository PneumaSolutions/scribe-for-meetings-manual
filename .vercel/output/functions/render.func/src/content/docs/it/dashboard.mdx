...
Se siete un controller, il modo più semplice per raggiungere la dashboard è attraverso l'interfaccia principale di RIM. Facendo clic sul pulsante RIM Dashboard si aprirà automaticamente la dashboard nel browser predefinito, con il login già effettuato.  
Se si è un amministratore di rete che non ha installato RIM, è sufficiente accedere al proprio account sul sito web di RIM per visualizzare la dashboard.

## Gestione degli obiettivi

Facendo clic sul link "Configura obiettivi", si accede a una pagina che consente di gestire tutte le macchine configurate per l'accesso non presidiato. È possibile fare clic su uno qualsiasi di questi computer per gestirlo. Una volta entrati, sarà possibile
...
Si noti che a tutti gli amministratori dell'organizzazione viene automaticamente concesso l'accesso alla gestione di tutti i gruppi creati nell'ambito dell'organizzazione.
...
## Gestione dell'abbonamento (piani PVVersonal e Pro)
