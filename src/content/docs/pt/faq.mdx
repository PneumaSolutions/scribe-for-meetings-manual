...
Isso pode se dever a qualquer uma das seguintes possibilidades:
...
* Se o computador de destino estiver usando o JAWS 2024, o processamento unificado do teclado às vezes interfere no uso da tecla CTRL para silenciar a fala. Embora tenhamos implementado uma correção para isso, às vezes ainda podem surgir problemas.
    ...
    * O processamento de teclado unificado não é necessário se o usuário-alvo também não estiver usando o ZoomText ou o Fusion.
* Se o computador de destino estiver usando o NVDA e tiver a opção "Manipular teclas de outros aplicativos" desmarcada, o RIM não poderá processar a entrada de teclado do controlador.
...
Isso pode ser devido à tecnologia Intel® Smart Sound para microfones digitais em alguns processadores Intel mais recentes. A desativação dessa tecnologia deve resolver o problema. Para desativar a tecnologia Intel® Smart Sound para microfones digitais, faça o seguinte:
...
1. Clique com o botão direito do mouse em "Tecnologia de som inteligente Intel® para microfones digitais" Escolha "Desativar dispositivo" no menu de contexto.

### O alvo está usando o ZoomText. Como posso experimentar a saída visual conforme renderizada por essa ampliação?

Isso é feito alternando o ZoomText para o modo de compatibilidade. Esse modo permite que os aprimoramentos de tela sejam compartilhados por meio de ferramentas como Teams ou Zoom, bem como RIM. Você pode ativá-lo no menu do ZoomText: Ajuda &gt; Avançar &gt; Adaptador de tela. Na caixa de diálogo aberta, selecione o modo de compatibilidade e o ZoomText será reiniciado. Recomendamos mudar o adaptador de vídeo de volta para o modo normal após a conclusão da sessão de RIM para otimizar o desempenho e a qualidade da ampliação. 

### Depois que um computador é recriado e reconfigurado com o mesmo nome de computador, há algo que precise ser feito para que o RIM volte a funcionar? 
Basta remover a máquina da sua conta, reinstalar o RIM e adicionar a máquina novamente à sua conta.
...
Processamos pagamentos por meio do FastSpring, portanto, podemos aceitar diretamente uma variedade de métodos de pagamento internacionais, incluindo os principais cartões de crédito, PayPal, Alipay e muitos outros.

### Como esses passes funcionam? O relógio começa imediatamente após o pagamento ou no dia em que eu iniciar a sessão?
Os passes só começam quando o controlador inicia a sessão. Portanto, se a máquina do alvo falhar, exigindo uma ida à oficina, e não for possível uma entrega no mesmo dia, você pode simplesmente esperar até que a máquina esteja de volta em boas condições e o seu passe diário ainda estará esperando por você.
...
Isso depende. Se a máquina estiver dentro da sua assinatura, ou seja, se você estiver acessando a máquina de sua casa enquanto estiver viajando, tudo correrá normalmente. Quaisquer outras conexões que não sejam o alvo inicial ao qual você se conectou funcionarão de acordo com a cota usual de 30 minutos.
...
### A máquina de destino foi reinicializada após a instalação de atualizações e drivers e requer uma senha para fazer login. Como a sessão continua a partir daqui?
